...
final class AuthModel {
  ...
  /// token (see ``randomizedRefreshDue(_:at:)``).
  var randomizedRefreshEnabled: Bool {
    didSet {
      guard randomizedRefreshEnabled != oldValue else { return }
      defaults.set(randomizedRefreshEnabled, forKey: Keys.randomizedRefreshEnabled)
      // The cadence rule just changed, so drop any pending backoff and the current
      // random target; the next tick reschedules the live token from scratch.
      refreshScheduler.reset()
      randomRefreshTargetFor = nil
    }
  }

  /// Whether refresh and auth events are appended to ``eventLogURL``. Off by
  /// default; the path is only shown in Settings while this is on.
  var logEventsEnabled: Bool {
    didSet {
  ...
  /// Where the opt-in event log is written — an autogenerated path in the temp
  /// directory, stable across launches so Settings can show and copy it.
  let eventLogURL: URL

  /// ``eventLogURL`` as a plain path, for display.
  var eventLogPath: String { eventLogURL.path(percentEncoded: false) }
  ...
  /// Appends refresh/auth events to disk when ``logEventsEnabled`` is on. Always
  /// present; the toggle gates whether ``logEvent(_:previous:refreshed:)`` calls it.
  private let eventLog: EventLog

  /// Successful refreshes since the current session was minted, written to the
  /// event log. Reset to zero whenever a new session begins.
  private var refreshCount = 0

  /// The wall-clock instant the current token's randomized refresh will fire at,
  /// with the `expiresAt` it was rolled for. Re-rolled when the token changes so a
  /// refreshed or replaced token gets a fresh random point. Consulted only while
  /// ``randomizedRefreshEnabled`` is on.
  private var randomRefreshTarget: Date?
  private var randomRefreshTargetFor: Date?
  ...
  private enum Keys {
    ...
    static let randomizedRefreshEnabled = "randomizedRefreshEnabled"
    static let logEventsEnabled = "logEventsEnabled"
  }
  ...
  init(defaults: UserDefaults = .standard) {
    ...
    // Both default to off — `bool(forKey:)` reads false for an absent key.
    self.randomizedRefreshEnabled = defaults.bool(forKey: Keys.randomizedRefreshEnabled)
    self.logEventsEnabled = defaults.bool(forKey: Keys.logEventsEnabled)
    self.eventLogURL = EventLog.defaultFileURL
    self.eventLog = EventLog(fileURL: eventLogURL)
    reload()
    startTicking()
  }
    ...
    private init(
      ...
      self.randomizedRefreshEnabled = false
      self.logEventsEnabled = false
      self.eventLogURL = EventLog.defaultFileURL
      self.eventLog = EventLog(fileURL: EventLog.defaultFileURL)
  ...
  private func autoRefreshIfNeeded(at instant: Date) {
    ...
    // The randomized schedule decides due-ness from a per-token random point; the
    // fixed schedule from the configured threshold. Either way the ceiling, backoff
    // and validity guards live in the scheduler.
    let due =
      randomizedRefreshEnabled
      ? randomizedRefreshDue(status, at: instant)
      : status.needsRefresh(at: instant, whenRemaining: refreshWhenRemaining)
    guard refreshScheduler.shouldAttempt(status, at: instant, due: due) else {
    ...
    work = Task { [weak self] in
      ...
      do {
        ...
        self.refreshCount += 1
        self.logEvent(.refresh, previous: previous, refreshed: refreshed)
  ...
  /// Whether the randomized schedule says `status` is due at `instant`.
  ///
  /// Rolls a fresh random target the first time it sees a given token (keyed by
  /// `expiresAt`, which every real refresh moves) and then answers from it, so the
  /// point is fixed for the life of that token instead of jumping every tick.
  private func randomizedRefreshDue(_ status: SessionStatus, at instant: Date) -> Bool {
    if randomRefreshTargetFor != status.expiresAt {
      var generator = SystemRandomNumberGenerator()
      let target = status.randomizedRefreshInstant(using: &generator)
      randomRefreshTarget = target
      randomRefreshTargetFor = status.expiresAt
      Self.logger.info(
        """
        Randomized refresh for \(status.profile, privacy: .public) scheduled at \
        \(target.formatted(date: .omitted, time: .standard), privacy: .public)
        """
      )
    }
    // A rolled target is always set here; the `?? instant` only guards the type.
    return instant >= (randomRefreshTarget ?? instant)
  }

  /// Appends one line to the opt-in event log, when it is on.
  ///
  ...
  private func renewSilently(
    ...
    do {
      ...
      refreshScheduler.reset()
      // Minting a fresh session from the API key restarts the tally.
      refreshCount = 0
      logEvent(.auth, previous: previous, refreshed: status)
      Self.logger.notice(
  ...
  private func mintNewSession(profile: String) async throws {
    ...
    // The token being replaced, for the event log; usually expired by this point.
    let previous = status
    ...
    notifiedExpiryFor = nil
    refreshScheduler.reset()
    // A new session starts the refresh tally over.
    refreshCount = 0
    logEvent(.auth, previous: previous, refreshed: status)
    Self.logger.notice(
      """
  ...
  func createProfile(named name: String, from source: OCIProfile?, region: String) async throws {
    ...
    profileName = name
    lastError = nil
    // A brand-new profile's session: no prior token, and the tally starts at zero.
    refreshCount = 0
    logEvent(.auth, previous: nil, refreshed: status)
    reload()
  }
