...
The menu bar stills at the top are regenerated by
`scripts/render-menu-bar-images.sh`, which draws each state through the app's own
`MenuBarRenderer` — so they cannot drift from what the status item really shows.
